...
# Message handler for edited m
@app.on_message(filters.group & filters.edited_message)
async def edited_message_handler(client, message):
    ...
    if edit_counts[user_id] >= 6:
        # Ban the user
        await client.kick_chat_member(chat_id=message.chat.id, user_id=user_id)
        await client.send_message(chat_id=message.chat.id, text=f"User {message.from_user.mention} has been banned for continuously editing messages.")
